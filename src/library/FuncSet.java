...
public class FuncSet extends NodeVector<Node> {
	
	private GPConfig config;

	private NodeVector<Node> theSet;

	public FuncSet(GPConfig conf) {
		config = conf;
	...
	public Function getRandomNode() {
		return (Function)theSet.getRandomElement();
	}

	public Function getRandomNode(int returnType) {
		return (Function)theSet.getRandomTypedElement(returnType);
	}
